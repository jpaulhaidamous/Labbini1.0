...
interface WhishInstructionsProps {
  amount: number;
  referenceNumber: string;
}

export default function WhishInstructions({ amount, referenceNumber }: WhishInstructionsProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Reference Number — prominent purple card */}
      <div className="bg-[#7C3AED] rounded-xl p-5 text-center">
      ...
      <div className="bg-[#F3F0FF] rounded-lg p-4 space-y-1.5">
        <div className="flex justify-between items-center">
          <span className="text-sm text-[#5A5A5A] font-medium">{t('amountUSD')}</span>
          <span className="font-bold text-[#7C3AED]">${amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
        </div>
        <div className="flex justify-between items-center">
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={i} className="flex gap-3">
            <div className="w-6 h-6 rounded-full bg-[#7C3AED] text-white flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
              {i + 1}
            </div>
      ...
      <div className="border border-[#E5DDD3] rounded-lg p-3 space-y-1.5">
        ...
        <div className="flex justify-between text-sm">
          <span className="text-[#5A5A5A]">{t('whishAccount')}</span>
          <span className="font-medium text-[#2C2C2C]">Labbini</span>
        </div>
      </div>
    </div>
  );
